...
Коллекция шаблонов, вырезанных из PHP-кода старого drive.ru, которые с помощью серверного рендеринга на новом drive.ru превращаются в готовые блоки для страниц. Шаблоны разделены на группы: каркасы страниц, контейнеры и блоки. Каркасы страниц содержат в себе верстку верхнего уровня, в которую после рендеринга вставляются все остальные шаблоны. Контейнеры представляют собой крупные фрагменты страниц, например заголовок и следующая за ним секция. Блоки — самые мелкие шаблоны, которые используются для отображения одинаковых элементов, например анонсов новостей, превью видео или иконок машин.

Данные для шаблонов можно получить через [API drive.ru](http://dev.drive.ru/docs/transition_api.html).

## Codestyle
В Handlebars expression может указываться либо имя поля в JSON, например `{{brand.id}}` или `{{caption}}`, вместо которого подставляются данные, либо плейсхолдер, вместо которого подставляется результат рендеринга другого JSON, например `{{{newsItems}}}`. Тройные фигурные скобки в Handlebars используются для вывода HTML без экранирования тегов. То есть, если то, что будет вставлено вместо Handlebars expression — это кусок HTML, то нужно писать его в тройных скобках.

Иногда удобно использовать Handlebars partials (`{{> foo}}`) — это вызов другого шаблона без изменения контекста.

Из helper'ов Handlebars используются только `if` и `each`. Подробнее о них можно прочитать в [документации Handlebars](http://handlebarsjs.com/).
...
Но шаблона `newsItems.hbs` не существует, а вместо выражения `{{{newsItems}}}` при рендеринге будет вставлен фрагмент HTML — смешанный список, получившийся из нескольких других шаблонов — `itemLarge.hbs`, `itemMedium.hbs` и `itemCompact.hbs`. Логика, по которой этот список строится, вынесена в рендедер, чтобы не загромождать ей шаблоны.
